...
async def run_pipeline(prompt: str) -> CompileResponse:
    """Execute the full 5-stage compiler pipeline with Railway timeout guard."""
    ...
    try:
        return await asyncio.wait_for(
            _run_pipeline_inner(prompt, job_id, start_time),
        ...
        return CompileResponse(
            ...
            metrics={
                "latency_ms": elapsed,
                "error": (
                    "Pipeline exceeded the 55-second time limit on Railway free tier. "
                    "Try a simpler prompt, or run locally for complex apps."
                ),
            },
        )
